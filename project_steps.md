...
### STEP 9: Node 2 - Swarm RAG Multi-Agent Search

**Purpose:**
- Find the best remediation script(s) for this incident
- Use multiple search strategies in parallel for maximum recall
- Apply weighted consensus to rank results

**What Happens (Layman View):**
Four different "search agents" simultaneously look for solutions: one searches by meaning (semantic), one by exact words (keyword), one by past relationships (graph), and one by metadata. They then vote on the best match.

**How It Works (Technical View):**
- **Protocol**: A2A (Agent-to-Agent) for coordination, MCP for tool calls
- **Sync/Async**: Asynchronous parallel execution
- **Architecture**: Swarm consensus pattern
...
| Agent | Protocol | Weight | Data Source | Search Method |
|-------|----------|--------|-------------|---------------|
| Vector Agent | MCP → Weaviate | 0.40 | Weaviate | Semantic similarity (all-MiniLM-L6-v2) |
| Keyword Agent | Internal | 0.25 | TF-IDF index | BM25 + bigram matching |
| Graph Agent | MCP → Neo4j | 0.25 | Neo4j | FIXED_BY relationship traversal |
| Metadata Agent | Internal | 0.10 | Script registry | Exact field matching |

**Sub-Step 9.3: Consensus Scoring**
```python
for script in all_results:
    score = (
        0.40 * vector_agent_score +
        0.25 * keyword_agent_score +
        0.25 * graph_agent_score +
        0.10 * metadata_agent_score
...
- **Reasoning Layer**: Consensus algorithm, cross-encoder model
...
- `backend/rag/agents/vector_agent.py` → Vector search agent
- `backend/rag/agents/keyword_agent.py` → Keyword search agent
- `backend/rag/agents/graph_agent.py` → Graph search agent
- `backend/rag/agents/metadata_agent.py` → Metadata search agent
- `backend/rag/cross_encoder_reranker.py:CrossEncoderReranker` → Reranking
- `backend/rag/hybrid_search_engine.py:HybridSearchEngine` → Search orchestration

**Failure & Fallback:**
- **What can fail**: Weaviate timeout, Neo4j connection error, embedding service failure
- **System reaction**:
  - Failed agent's vote is excluded (remaining agents continue)
...
### 5. Swarm Intelligence (RAG)
**What**: Multiple agents vote on decisions
**Why**: Improves accuracy through consensus
**Where**: Swarm RAG with 4 specialized search agents
...
| **Swarm RAG** | Multi-agent retrieval system using weighted consensus |
